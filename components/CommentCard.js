...
export default function ({ data, del, getReply, updateComment, activate}) {
    ...
    async function updatehandeler(event, id,commentid) {
        ...
        updateComment(dataToSave,commentid,id)
        setVisable(false)
        event.target.reset();
    }
    ...
    return (
        <div className="oneComment">
            ...
            <div className="content">
                            ...
                            <Menu as="div" className="relative inline-block text-left">
                                ...
                                <Transition
                                    ...
                                    <Menu.Items className="absolute right-0 z-10 mt-2 w-40 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                        ...
                                        <div className="py-1">
                                            <Menu.Item className="choice" onClick={() => (setOpen(true))} >
                                                {({ active }) => (
                                                    <a className={
                ...
                {visable ? (
                    <form className="update" method="post" onSubmit={(event)=>updatehandeler(event,data.project.id,data.id)} >
                        <textarea name="comme" placeholder="update the comment" ></textarea>
                    ...
                    <>
                        {data.body}
                        <div className="button">
                            <button onClick={() => (getReply(data.id), activate(data.id))}  ><strong>Reply</strong></button>
                        </div>
                    </>
                )}
            </div>
                                ...
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                            ...
                                            <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                ...
                                                <div className="mt-2">
                                                    <p className="text-sm text-gray-500">
                                                        Are you sure you want to delete your comment? All of your reply will be permanently
                                    ...
                                    <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                        <button
                                            ...
                                            onClick={() => (setOpen(false), del(data.id, data.project.id))}
                                        >
                                            Delete
                                        </button>
                                        <button
